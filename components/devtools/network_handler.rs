...
pub(crate) fn handle_network_event(
    ...
    let watcher = actors.find::<WatcherActor>(&actor.watcher_name);

    match network_event {
        NetworkEvent::HttpRequest(httprequest) => {
            actor.set_request(httprequest);
        ...
        NetworkEvent::HttpResponse(httpresponse) => {
            actor.set_response(httpresponse);
            let resource = actor.resource_updates();
        ...
        NetworkEvent::SecurityInfo(update) => {
            actor.set_security_info(update.security_info);
            let resource = actor.resource_updates();
