...
class CommandBase(object):
        ...
        if build_configuration:
            decorators += [
                ...
                CommandArgument(
                    ...
                    group="Feature Selection",
                    action="store_true",
                    help="Use the GStreamer installed on the system instead of bundling a pre-built version",
                ),
                CommandArgument(
                    "--debug-mozjs",
                    default=False,
    ...
    def is_media_enabled(self, media_stack: Optional[str]) -> bool:
        ...
        # This is only relevant when opting out of the bundled GStreamer.
        platform = servo.platform.get()
        if (
            not self.target.is_cross_build()
            and platform.is_linux
            and self.use_system_gstreamer
            and not platform.is_gstreamer_installed(self.target)
        ):
            return False
